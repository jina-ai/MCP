...
| `expand_query` | Rewrite a query into several diverse queries via [Reader API](https://jina.ai/reader) | Yes |
| `parallel_read_url` | Read multiple web pages in parallel via [Reader API](https://jina.ai/reader) | Optional* |
| `parallel_search_web` | Run multiple web searches in parallel via [Reader API](https://jina.ai/reader) | Yes |
| `parallel_search_arxiv` | Run multiple arXiv searches in parallel via [Reader API](https://jina.ai/reader) | Yes |
| `parallel_search_ssrn` | Run multiple SSRN searches in parallel via [Reader API](https://jina.ai/reader) | Yes |
...
Claude Code prefers `parallel_*` tools (`parallel_search_web`, `parallel_read_url`); models like Qwen3-Next prefer singleton tools called with an array. Both work: the singletons (`search_web`, `search_arxiv`, `search_ssrn`, `read_url`) take a string or an array on `query`/`url`, and an array runs every query concurrently, matching `parallel_*` behavior. Use whichever the model prefers. Arrays cap at 5 entries, the same limit `parallel_*` enforces.
...
Off by default, because OCR bills more tokens per page than a plain read costs per document. Turn it on when the HTML path fails or the layout matters. `parallel_read_url` takes both flags per entry: repeat the same url with different `page` values.
